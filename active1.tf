resource "azurerm_linux_virtual_machine" "activefgtvm1" {
...
resource "azurerm_virtual_machine_data_disk_attachment" "res-4" {
  ...
  virtual_machine_id = azurerm_linux_virtual_machine.activefgtvm1.id
  depends_on = [
    azurerm_resource_group.res-0,
    azurerm_managed_disk.res-1,
    azurerm_linux_virtual_machine.activefgtvm1,
  ]
}